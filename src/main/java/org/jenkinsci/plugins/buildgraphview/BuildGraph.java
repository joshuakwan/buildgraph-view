...
public class BuildGraph implements Action {
	...
	private static final Logger LOGGER = Logger.getLogger(BuildGraph.class
			.getName());
	...
	public BuildGraph(AbstractBuild run) {
		LOGGER.log(Level.INFO, "Construct BuildGraph from " + run.getUrl());
	...
	public DirectedGraph<BuildExecution, Edge> getGraph()
		...
		LOGGER.log(Level.INFO, "Enter BuildGraph.getGraph()");
	...
	private void computeGraphFrom(BuildExecution b) throws ExecutionException,
			InterruptedException {
		Run run = b.getBuild();
		LOGGER.log(Level.INFO, "Enter BuildGraph.computeGraphFrom() from " + run.getUrl());
		for (DownStreamRunDeclarer declarer : DownStreamRunDeclarer.all()) {
			List<Run> runs = declarer.getDownStream(run);
			LOGGER.log(Level.INFO, "BuildGraph.computeGraphFrom() - Get Downstreams ");
			for (Run r : runs) {
				BuildExecution next = getExecution(r);
				LOGGER.log(Level.INFO, next.getBuildUrl());
	...
	private BuildExecution getExecution(Run r) {
		for (BuildExecution buildExecution : graph.vertexSet()) {
			if (buildExecution.getBuild().equals(r))
	...
	private void setupDisplayGrid() {
		...
		Collections.sort(allPaths, new Comparator<List<BuildExecution>>() {
			public int compare(List<BuildExecution> runs1,
					List<BuildExecution> runs2) {
	...
	private List<List<BuildExecution>> findAllPaths(BuildExecution start) {
		...
		if (graph.outDegreeOf(start) == 0) {
			...
			for (Edge edge : graph.outgoingEdgesOf(start)) {
				List<List<BuildExecution>> allPathsFromTarget = findAllPaths(edge
						.getTarget());
